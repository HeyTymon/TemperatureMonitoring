...
IPAddress staticIP(172, 16, 1, 57);
...
bool checkNameAndIP() {
  String data = "name=Sensor57&ip=" + WiFi.localIP().toString();
  Serial.println(data);
  HTTPClient http;
  http.begin(URL2);
...
void sendData(void) {
  String data = "temperature=" + String(bme.readTemperature()) + "&humidity=" + String(bme.readHumidity()) + "&sensorName=Sensor57";
  HTTPClient http;
  http.begin(URL);
