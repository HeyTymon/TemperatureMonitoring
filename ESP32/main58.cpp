...
bool checkNameAndIP() {
  String data = "name=Sensor58&ip=" + WiFi.localIP().toString();
  Serial.println(data);
  HTTPClient http;
  http.begin(URL2);
...
void sendData(void) {
  String data = "temperature=" + String(bme.readTemperature()) + "&humidity=" + String(bme.readHumidity()) + "&sensorName=Sensor58";
  HTTPClient http;
  http.begin(URL);
